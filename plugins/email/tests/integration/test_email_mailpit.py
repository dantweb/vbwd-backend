    ...
    while time.monotonic() < deadline:
        ...
        for msg in messages:
            ...
            if to in recipients:
                ...
                msg_id = msg["ID"]
                full = requests.get(f"{MAILPIT_API}/api/v1/message/{msg_id}", timeout=3).json()
                return full
        time.sleep(poll)
    return None
...
def _seed(app, db, event_type: str, subject_tpl: str, html_tpl: str, text_tpl: str = "") -> None:
    with app.app_context():
        existing = db.session.query(EmailTemplate).filter_by(event_type=event_type).first()
        if existing:
...
class TestUserRegisteredEmail:
    ...
    def test_subject_rendered_with_name(self, app, db):
        _seed(app, db,
              ...
              "<p>Hi</p>")
        bus = _bus(app)
        bus.publish("user.registered", {"user_email": "bob@vbwd.test", "user_name": "Bob"})
    ...
    def test_html_body_contains_user_name(self, app, db):
        ...
        bus = _bus(app)
        bus.publish("user.registered", {"user_email": "carol@vbwd.test", "user_name": "Carol"})
...
class TestUserPasswordResetEmail:
    def test_reset_email_delivered(self, app, db):
        ...
        bus = _bus(app)
        bus.publish("user.password_reset", {
...
class TestSubscriptionActivatedEmail:
    def test_email_delivered(self, app, db):
        _seed(app, db,
              ...
              "<p>Plan: {{ plan_name }}</p>")
        bus = _bus(app)
        bus.publish("subscription.activated", {
...
class TestSubscriptionCancelledEmail:
    def test_email_delivered(self, app, db):
        ...
        bus = _bus(app)
        bus.publish("subscription.cancelled", {
...
class TestSubscriptionPaymentFailedEmail:
    def test_email_delivered(self, app, db):
        _seed(app, db,
              ...
              "<p>Retry soon</p>")
        bus = _bus(app)
        bus.publish("subscription.payment_failed", {
...
class TestContactFormEmail:
    def test_email_delivered_to_recipient(self, app, db):
        _seed(app, db,
              ...
              "Form: {{ widget_slug }}\n{{ fields_text }}")
        bus = _bus(app)
        bus.publish("contact_form.received", {
...
class TestSubscriptionRenewedEmail:
    def test_email_delivered(self, app, db):
        ...
        bus = _bus(app)
        bus.publish("subscription.renewed", {
