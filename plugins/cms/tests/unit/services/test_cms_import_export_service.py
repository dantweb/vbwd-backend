...
def _make_widget(slug="w1", name="Widget 1", wtype="html"):
    ...
    w = CmsWidget()
    w.id = uuid4(); w.slug = slug; w.name = name
    w.widget_type = wtype; w.content_json = {}; w.source_css = None
    w.config = None; w.sort_order = 0; w.is_active = True
...
def _make_category(slug="cat1", name="Cat 1"):
    ...
    c = CmsCategory()
    c.id = uuid4(); c.slug = slug; c.name = name
    c.parent_id = None; c.sort_order = 0
...
def _make_style(slug="sty1", name="Style 1"):
    ...
    s = CmsStyle()
    s.id = uuid4(); s.slug = slug; s.name = name
    s.source_css = ".a{}"; s.sort_order = 0; s.is_active = True
...
def _make_layout(slug="lay1", name="Layout 1"):
    ...
    lay = CmsLayout()
    lay.id = uuid4(); lay.slug = slug; lay.name = name
    lay.description = ""; lay.areas = []; lay.sort_order = 0; lay.is_active = True
...
def _make_page(slug="pg1", name="Page 1"):
    ...
    p = CmsPage()
    p.id = uuid4(); p.slug = slug; p.name = name
    p.language = "en"; p.content_json = {}; p.content_html = None
    p.source_css = None; p.category_id = None; p.layout_id = None
    p.style_id = None; p.is_published = False; p.sort_order = 0
    p.meta_title = None; p.meta_description = None; p.meta_keywords = None
    p.og_title = None; p.og_description = None; p.og_image_url = None
    p.canonical_url = None; p.robots = "index,follow"; p.schema_json = None
...
def _make_routing_rule(name="Rule 1"):
    ...
    r = CmsRoutingRule()
    r.id = uuid4(); r.name = name; r.is_active = True; r.priority = 0
    r.match_type = "default"; r.match_value = None; r.target_slug = "home"
    r.redirect_code = 302; r.is_rewrite = False; r.layer = "middleware"
...
def _make_image(slug="img1"):
    ...
    img = CmsImage()
    img.id = uuid4(); img.slug = slug; img.caption = "cap"
    img.file_path = f"{slug}.jpg"; img.url_path = f"/uploads/{slug}.jpg"
    img.mime_type = "image/jpeg"; img.file_size_bytes = 100
    img.width_px = 800; img.height_px = 600; img.alt_text = "alt"
    img.created_at = img.updated_at = datetime.datetime.utcnow()
    return img


def _paginated(items):
    return {"items": items, "total": len(items), "page": 1, "per_page": 100000, "pages": 1}


def _make_svc(
    categories=None, styles=None, widgets=None, layouts=None,
    pages=None, routing_rules=None, images=None,
    lw_assignments=None, storage=None,
):
    cat_repo = MagicMock()
    ...
    return CmsImportExportService(
        cat_repo, style_repo, widget_repo, layout_repo,
        page_repo, routing_repo, image_repo, lw_repo, fs,
    ), cat_repo, style_repo, widget_repo, layout_repo, page_repo, routing_repo, image_repo, lw_repo
...
def _make_zip(**section_dicts) -> bytes:
    ...
    manifest = {"version": "1.0", "exported_at": "2026-01-01T00:00:00+00:00", "sections": sections}
    buf = io.BytesIO()
...
class TestImportAdd:
    def test_import_add_inserts_new_widget(self):
        data = _make_zip(widgets=[{"slug": "new-w", "name": "New", "widget_type": "html",
                                   "content_json": {}, "source_css": None, "config": None,
    ...
    def test_import_add_skips_existing_slug(self):
        ...
        data = _make_zip(widgets=[{"slug": "existing-w", "name": "X", "widget_type": "html",
                                   "content_json": {}, "source_css": None, "config": None,
                                   "sort_order": 0, "is_active": True}])
    ...
    def test_import_add_inserts_categories(self):
        data = _make_zip(categories=[{"slug": "software", "name": "Software", "sort_order": 0}])
    ...
    def test_import_returns_no_errors_on_success(self):
        data = _make_zip(widgets=[{"slug": "w-ok", "name": "OK", "widget_type": "html",
                                   "content_json": {}, "source_css": None, "config": None,
                                   "sort_order": 0, "is_active": True}])
        svc, *_ = _make_svc()
        result = svc.import_zip(data, "add")
...
class TestImportIndex:
    def test_import_index_renames_slug_on_conflict(self):
        ...
        data = _make_zip(widgets=[{"slug": "breadcrumbs", "name": "BC", "widget_type": "html",
                                   "content_json": {}, "source_css": None, "config": None,
                                   "sort_order": 0, "is_active": True}])
        svc, _, _, widget_repo, *_ = _make_svc(widgets=[existing])
    ...
    def test_import_index_increments_to_next_free_suffix(self):
        ...
        data = _make_zip(widgets=[{"slug": "nav", "name": "Nav", "widget_type": "html",
                                   "content_json": {}, "source_css": None, "config": None,
                                   "sort_order": 0, "is_active": True}])
    ...
    def test_import_index_inserts_non_conflicting_as_is(self):
        data = _make_zip(widgets=[{"slug": "unique-slug", "name": "U", "widget_type": "html",
                                   "content_json": {}, "source_css": None, "config": None,
                                   "sort_order": 0, "is_active": True}])
        svc, _, _, widget_repo, *_ = _make_svc()
        result = svc.import_zip(data, "index")
...
class TestImportDropAll:
    def test_drop_all_calls_bulk_delete_for_widgets(self):
        ...
        data = _make_zip(widgets=[{"slug": "new-w", "name": "N", "widget_type": "html",
                                   "content_json": {}, "source_css": None, "config": None,
                                   "sort_order": 0, "is_active": True}])
        svc, _, _, widget_repo, *_ = _make_svc(widgets=[existing])
    ...
    def test_drop_all_inserts_all_records_after_clear(self):
        ...
        data = _make_zip(widgets=[
            {"slug": "hero", "name": "Hero New", "widget_type": "html",
             "content_json": {}, "source_css": None, "config": None,
             "sort_order": 0, "is_active": True},
        ])
        svc, _, _, widget_repo, *_ = _make_svc(widgets=[existing])
    ...
    def test_drop_all_calls_delete_for_routing_rules(self):
        ...
        data = _make_zip(routing_rules=[{"name": "Rule B", "is_active": True, "priority": 0,
                                         "match_type": "default", "match_value": None,
                                         "target_slug": "home", "redirect_code": 302,
...
class TestImportPages:
    def test_import_page_resolves_category_slug(self):
        ...
        data = _make_zip(pages=[{
            "slug": "my-page", "name": "My Page", "language": "en",
            "content_json": {}, "content_html": None, "source_css": None,
            "category_id": None, "layout_id": None, "style_id": None,
            ...
            "canonical_url": None, "robots": "index,follow", "schema_json": None,
            "category_slug": "software", "layout_slug": None, "style_slug": None,
        }])
    ...
    def test_import_page_sets_null_fk_for_missing_layout(self):
        data = _make_zip(pages=[{
            "slug": "pg-no-layout", "name": "P", "language": "en",
            "content_json": {}, "content_html": None, "source_css": None,
            "category_id": None, "layout_id": None, "style_id": None,
            "is_published": False, "sort_order": 0,
            "meta_title": None, "meta_description": None, "meta_keywords": None,
            "og_title": None, "og_description": None, "og_image_url": None,
            "canonical_url": None, "robots": "index,follow", "schema_json": None,
            "category_slug": None, "layout_slug": "missing-layout", "style_slug": None,
        }])
    ...
    def test_import_page_skips_on_slug_conflict_with_add(self):
        ...
        data = _make_zip(pages=[{
            "slug": "home", "name": "Home", "language": "en",
            "content_json": {}, "content_html": None, "source_css": None,
            "category_id": None, "layout_id": None, "style_id": None,
            ...
            "og_title": None, "og_description": None, "og_image_url": None,
            "canonical_url": None, "robots": "index,follow", "schema_json": None,
            "category_slug": None, "layout_slug": None, "style_slug": None,
        }])
...
class TestImportLayouts:
    def test_import_layout_creates_widget_assignments(self):
        ...
        data = _make_zip(layouts=[{
            "slug": "main-layout", "name": "Main", "description": "",
            "areas": ["header", "main"], "sort_order": 0, "is_active": True,
            "widget_assignments": [
                {"widget_slug": "hero-widget", "area_name": "header", "sort_order": 0},
            ],
        }])
    ...
    def test_import_layout_skips_missing_widget_slug(self):
        data = _make_zip(layouts=[{
            "slug": "layout-x", "name": "X", "description": "",
            "areas": [], "sort_order": 0, "is_active": True,
            "widget_assignments": [
                {"widget_slug": "nonexistent", "area_name": "main", "sort_order": 0},
            ],
        }])
...
class TestImportImages:
    def test_import_image_saves_binary_to_storage(self):
        ...
        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr("manifest.json", '{"version":"1.0","exported_at":"","sections":["images"]}')
            zf.writestr("images.json", json.dumps([{
                "slug": "logo", "caption": "logo", "file_path": "logo.png",
                "url_path": "/uploads/logo.png", "mime_type": "image/png",
                "file_size_bytes": 6, "width_px": 100, "height_px": 50, "alt_text": "logo",
            }]))
    ...
    def test_import_image_metadata_only_when_no_binary(self):
        data = _make_zip(images=[{
            "slug": "remote-img", "caption": "", "file_path": "remote.jpg",
            "url_path": "/uploads/remote.jpg", "mime_type": "image/jpeg",
            "file_size_bytes": 0, "width_px": None, "height_px": None, "alt_text": "",
        }])
...
class TestImportRoutingRules:
    def test_import_routing_rule_add_skips_existing_name(self):
        ...
        data = _make_zip(routing_rules=[{
            "name": "Default Route", "is_active": True, "priority": 0,
            "match_type": "default", "match_value": None, "target_slug": "home",
