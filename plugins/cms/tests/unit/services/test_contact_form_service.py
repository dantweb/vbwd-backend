...
class TestHoneypot:
    def test_empty_honeypot_passes(self):
        svc = _make_service()
        result = svc.process_submission(
            config=BASE_CONFIG, form_data=BASE_DATA, remote_ip="1.2.3.4"
        )
    ...
    def test_filled_honeypot_raises(self):
        ...
        with pytest.raises(HoneypotError):
            svc.process_submission(
                config=BASE_CONFIG, form_data=data, remote_ip="1.2.3.4"
            )
...
class TestRateLimit:
    ...
    def test_rate_limit_disabled_ignores_count(self):
        ...
        result = svc.process_submission(
            config=cfg, form_data=BASE_DATA, remote_ip="1.2.3.4"
        )
        assert result is not None
...
class TestValidation:
    ...
    def test_invalid_url_raises(self):
        ...
        data = {
            **BASE_DATA,
            "fields": {"name": "Bob", "email": "b@b.com", "site": "ftp://bad"},
        }
...
class TestSanitization:
    ...
    def test_value_truncated_to_max_length(self):
        ...
        data = {
            **BASE_DATA,
            "fields": {"name": "Bob", "email": "b@b.com", "message": long_value},
        }
        result = svc.process_submission(
            config=BASE_CONFIG, form_data=data, remote_ip="1.2.3.4"
        )
...
class TestFieldTypes:
    def test_radio_valid_option(self):
        ...
        data = {
            **BASE_DATA,
            "fields": {"name": "Bob", "email": "b@b.com", "topic": "Support"},
        }
        result = svc.process_submission(config=cfg, form_data=data, remote_ip="1.2.3.4")
    ...
    def test_radio_invalid_option_raises(self):
        ...
        data = {
            **BASE_DATA,
            "fields": {"name": "Bob", "email": "b@b.com", "topic": "Hacked"},
        }
...
class TestPayloadShape:
    def test_payload_contains_expected_keys(self):
        svc = _make_service()
        result = svc.process_submission(
            config=BASE_CONFIG, form_data=BASE_DATA, remote_ip="10.0.0.1"
        )
        assert result["widget_slug"] == "contact-widget"
