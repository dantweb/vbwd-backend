...
class GhrmSoftwarePackageRepository:
    ...
    def find_by_slug(self, slug: str) -> Optional[GhrmSoftwarePackage]:
        return self.session.query(GhrmSoftwarePackage).filter(GhrmSoftwarePackage.slug == slug).first()
    ...
    def find_by_slugs(self, slugs: List[str]) -> List[GhrmSoftwarePackage]:
        return self.session.query(GhrmSoftwarePackage).filter(GhrmSoftwarePackage.slug.in_(slugs)).all()
    ...
    def find_all(self, page: int = 1, per_page: int = 20, category_slug: Optional[str] = None, query: Optional[str] = None) -> Dict[str, Any]:
        ...
        total = q.count()
        items = q.order_by(GhrmSoftwarePackage.sort_order.asc(), GhrmSoftwarePackage.name.asc()).offset((page - 1) * per_page).limit(per_page).all()
    ...
    def increment_downloads(self, slug: str) -> None:
        self.session.query(GhrmSoftwarePackage).filter(GhrmSoftwarePackage.slug == slug).update(
            {GhrmSoftwarePackage.download_counter: GhrmSoftwarePackage.download_counter + 1}
        )
        self.session.flush()
        self.session.commit()
