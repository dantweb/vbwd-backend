...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("tariff_plan_id", sa.UUID(), sa.ForeignKey("tarif_plan.id", ondelete="CASCADE"), nullable=False),
        ...
        sa.Column("github_protected_branch", sa.String(64), nullable=False, server_default="release"),
    ...
    op.create_index("ix_ghrm_software_package_slug", "ghrm_software_package", ["slug"], unique=True)

    # ── ghrm_software_sync ────────────────────────────────────────────────────
    op.create_table(
        "ghrm_software_sync",
        sa.Column("id", sa.UUID(), nullable=False),
        sa.Column("software_package_id", sa.UUID(), sa.ForeignKey("ghrm_software_package.id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_index("ix_ghrm_software_sync_package_id", "ghrm_software_sync", ["software_package_id"], unique=True)
    ...
    op.create_index("ix_ghrm_user_github_access_user_id", "ghrm_user_github_access", ["user_id"], unique=True)
    ...
    op.create_table(
        ...
        sa.Column("package_id", sa.UUID(), sa.ForeignKey("ghrm_software_package.id", ondelete="SET NULL"), nullable=True),
    ...
    op.create_index("ix_ghrm_access_log_user_id", "ghrm_access_log", ["user_id"], unique=False)
    op.create_index("ix_ghrm_access_log_package_id", "ghrm_access_log", ["package_id"], unique=False)
