...
def upgrade() -> None:
    ...
    op.alter_column(
        ...
        existing_type=postgresql.ENUM(
            "MAJOR_ARCANA", "CUPS", "WANDS", "SWORDS", "PENTACLES", name="arcanatype"
        ),
    ...
    op.drop_constraint(
        "feature_usage_user_id_fkey", "feature_usage", type_="foreignkey"
    )
    op.create_foreign_key(None, "feature_usage", "user", ["user_id"], ["id"])
    op.drop_constraint(
        "uq_password_reset_token_token", "password_reset_token", type_="unique"
    )
    ...
    op.drop_constraint(
        "role_permissions_role_id_fkey", "role_permissions", type_="foreignkey"
    )
    op.drop_constraint(
    ...
    op.create_foreign_key(
        None, "role_permissions", "permission", ["permission_id"], ["id"]
    )
    op.drop_index("ix_subscription_stripe_subscription_id", table_name="subscription")
    op.drop_constraint(
        "uq_subscription_stripe_subscription_id", "subscription", type_="unique"
    )
    op.create_index(
        op.f("ix_subscription_provider_subscription_id"),
        "subscription",
    ...
    op.drop_constraint(
        "taro_card_draw_arcana_id_fkey", "taro_card_draw", type_="foreignkey"
    )
    op.drop_constraint(
        "taro_card_draw_session_id_fkey", "taro_card_draw", type_="foreignkey"
    )
    op.alter_column(
        "taro_session",
        "status",
        existing_type=postgresql.ENUM(
            "ACTIVE", "EXPIRED", "CLOSED", name="tarosessionstatus"
        ),
        type_=sa.String(length=50),
        existing_nullable=False,
    ...
    op.add_column(
        "user",
        sa.Column(
            "daily_taro_sessions_used", sa.Integer(), nullable=True, server_default="0"
        ),
    )
    op.add_column(
        "user",
        sa.Column("daily_taro_limit", sa.Integer(), nullable=True, server_default="3"),
    )
    # Update existing rows to have default values
    op.execute(
        'UPDATE "user" SET daily_taro_sessions_used = 0 WHERE daily_taro_sessions_used IS NULL'
    )
    op.execute('UPDATE "user" SET daily_taro_limit = 3 WHERE daily_taro_limit IS NULL')
    # Make columns non-nullable
    op.alter_column(
        "user", "daily_taro_sessions_used", existing_type=sa.Integer(), nullable=False
    )
    op.alter_column(
        "user", "daily_taro_limit", existing_type=sa.Integer(), nullable=False
    )
    op.drop_index("ix_user_stripe_customer_id", table_name="user")
    op.drop_constraint("uq_user_stripe_customer_id", "user", type_="unique")
    op.create_index(
        op.f("ix_user_payment_customer_id"),
        "user",
        ["payment_customer_id"],
        unique=True,
    )
    ...
    op.drop_constraint(
        "uq_user_invoice_stripe_invoice_id", "user_invoice", type_="unique"
    )
    op.create_index(
        op.f("ix_user_invoice_provider_session_id"),
        "user_invoice",
        ["provider_session_id"],
        unique=True,
    )
    ...
    op.drop_constraint(
        "uq_user_token_balance_user_id", "user_token_balance", type_="unique"
    )
...
def downgrade() -> None:
    ...
    op.create_unique_constraint(
        "uq_user_token_balance_user_id", "user_token_balance", ["user_id"]
    )
    ...
    op.drop_index(
        op.f("ix_user_invoice_provider_session_id"), table_name="user_invoice"
    )
    op.create_unique_constraint(
        "uq_user_invoice_stripe_invoice_id", "user_invoice", ["provider_session_id"]
    )
    op.create_index(
        "ix_user_invoice_stripe_invoice_id",
        "user_invoice",
        ["provider_session_id"],
        unique=False,
    )
    ...
    op.create_unique_constraint(
        "uq_user_stripe_customer_id", "user", ["payment_customer_id"]
    )
    op.create_index(
        "ix_user_stripe_customer_id", "user", ["payment_customer_id"], unique=False
    )
    ...
    op.create_foreign_key(
        "taro_card_draw_session_id_fkey",
        "taro_card_draw",
        "taro_session",
        ["session_id"],
        ["id"],
    ...
    op.drop_index(
        op.f("ix_subscription_provider_subscription_id"), table_name="subscription"
    )
    op.create_unique_constraint(
    ...
    op.create_unique_constraint(
        "uq_password_reset_token_token", "password_reset_token", ["token"]
    )
    op.drop_constraint(None, "feature_usage", type_="foreignkey")
    op.create_foreign_key(
        "feature_usage_user_id_fkey",
        "feature_usage",
        "user",
        ["user_id"],
        ["id"],
    ...
    op.create_unique_constraint(
        "uq_arcana_number_suit_rank", "arcana", ["number", "suit", "rank"]
    )
    op.alter_column(
        "arcana",
        "arcana_type",
    ...
    op.create_table(
        ...
        sa.Column(
            "plugin_name", sa.VARCHAR(length=100), autoincrement=False, nullable=False
        ),
        sa.Column(
            "status",
            sa.VARCHAR(length=20),
            server_default=sa.text("'disabled'::character varying"),
            autoincrement=False,
            nullable=False,
        ),
        sa.Column(
        ...
        sa.Column(
            "enabled_at", postgresql.TIMESTAMP(), autoincrement=False, nullable=True
        ),
        sa.Column(
            "disabled_at", postgresql.TIMESTAMP(), autoincrement=False, nullable=True
        ),
        sa.Column(
            "created_at",
    ...
    op.create_index(
        "ix_plugin_config_plugin_name", "plugin_config", ["plugin_name"], unique=False
    )
    # ### end Alembic commands ###
