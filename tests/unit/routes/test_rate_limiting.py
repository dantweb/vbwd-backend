...
class TestRateLimiting:
    ...
    def test_login_allows_requests_under_limit(self, mock_auth_service, mock_repo, client):
        """Login allows requests under the rate limit."""
        # Mock auth service to return failure (invalid credentials)
        mock_instance = MagicMock()
        mock_instance.login.return_value = MagicMock(success=False, error="Invalid credentials")
    ...
    @patch('src.routes.auth.UserRepository')
    @patch('src.routes.auth.AuthService')
    def test_login_rate_limited_after_exceeded(self, mock_auth_service, mock_repo, client):
    ...
    @patch('src.routes.auth.UserRepository')
    @patch('src.routes.auth.AuthService')
    def test_rate_limit_response_includes_retry_after(self, mock_auth_service, mock_repo, client):
